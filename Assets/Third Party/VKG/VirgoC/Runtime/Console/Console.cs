...
namespace DeveloperConsole
{
    public class Console : MonoBehaviour
    {
         ...
         */
        public Key keyOpen = Key.Backquote;

        [Tooltip("Which key to use to open the console.")]
        /**
         * Choose on the inspector which key to use to open the console.
         */
        public Key keyOpenAlt = Key.Quote;
        ...
        void ProcessInput()
        {
            ...
            {
                if (keyboard[keyOpen].wasPressedThisFrame || keyboard[keyOpenAlt].wasPressedThisFrame)
                {
                    Toggle();
                }
                if (keyboard[keyType].wasPressedThisFrame && !isOpen)
                {
                    Open();
                }
            }

            if (keyboard[keyType].wasPressedThisFrame)
            {
                inputField.Select();
            }
            ...
            if (mouse != null && mouse.leftButton.wasReleasedThisFrame && isResizing)
            {
                StopResizing();
            }
        }
        ...
        void StartResizing()
        {
            ...
            resizeInitialMousePosition = Mouse.current != null ? Mouse.current.position.ReadValue() : Vector2.zero;
        ...
        IEnumerator Resize()
        {
            while (true)
            {
                if (isResizing)
                {
                    Vector2 mousePosition = Mouse.current != null ? Mouse.current.position.ReadValue() : resizeInitialMousePosition;
                    Vector2 mouseOffset = mousePosition - resizeInitialMousePosition;
